...
func (a *Application) Start() {
	...
	logger, err := a.initLogger(cfg.Logger.Level, cfg.Logger.IsJSON)
	if err != nil {
	...
	a.initSignalHandler(ctx, logger, cancelFunc)
	...
	db, err := a.initDb(cfg.DB.Host, cfg.DB.Port, cfg.DB.User, cfg.DB.Password, cfg.DB.Name)
	if err != nil {
	...
	go func() {
		defer wg.Done()
		err = a.startGRPCServer(ctx, cfg.App.GrpcListen, db, logger)
		if err != nil {
...
func (a *Application) initConfig() (config.Config, error) {
...
func (a *Application) initDb(host string, port int, user string, password string, dbname string) (*sqlx.DB, error) {
...
func (a *Application) migrateUp(db *sqlx.DB) error {
...
func (a *Application) initLogger(logLevel string, isLogJson bool) (*zap.Logger, error) {
	lvl := zap.InfoLevel
...
func (a *Application) initSignalHandler(
	ctx context.Context,
	logger *zap.Logger,
